...
async function callGenerateQuizApi(
  ...
  try {
    const response = await fetch("/api/generate-quiz", {
      ...
      body: JSON.stringify({ content, preferThai, difficulty, questionCount }),
    });
...
> {
  ...
  const rawQuestions = await callGenerateQuizApi(content, preferThai, difficulty, questionCount);
  ...
  const session: GeneratedQuizSession = {
    ...
    description: preferThai
      ? `ควิซระดับ${difficultyLabel[difficulty]} ${questions.length} ข้อ สร้างจากไฟล์ที่อัปโหลด โดย Gemini AI`
      : `${questions.length}-question ${difficulty} quiz generated by Gemini AI from your uploaded file content.`,
